...
interface ProfesionalPageProps {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(
        (p) =>
          String(p.personaId).includes(q) ||
          (p.cargoNombre || '').toLowerCase().includes(q) ||
          (p.estadoDescripcion || '').toLowerCase().includes(q) ||
          (p.tipoContratoNombre || '').toLowerCase().includes(q) ||
          (p.afpNombre || '').toLowerCase().includes(q) ||
          (p.previsionNombre || '').toLowerCase().includes(q)
      );
    }

    if (filterActivo !== 'all') {
      result = result.filter((p) => String(p.activo) === filterActivo);
    }

    return result;
  }, [profesionales, search, filterActivo]);
